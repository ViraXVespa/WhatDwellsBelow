...
func _ready() -> void:
	...
	if App.character_chosen:
		...
		card.add_child(_btn("Play", func(): _play(App.character_type)))
	else:
		...
		card.add_child(row)
		row.add_child(_btn("Play — Male", func(): _play("male")))
		row.add_child(_btn("Play — Female", func(): _play("female")))
	card.add_child(_btn("Archives", _open_archives))
...
func _lab(text: String, size: int, col: Color) -> Label:
	...
	l.add_theme_constant_override("outline_size", 8 if size >= 40 else 5)
	return l
