...
## Baked game version + current-series changelog. Web overlay prefers window.__wdbNotes.
...
static func _web_notes_payload() -> Dictionary:
    ...
    if not ClassDB.class_exists("JavaScriptBridge"):
        return {}
    var raw: Variant = JavaScriptBridge.eval("JSON.stringify(window.__wdbNotes || null)")
...
static func _read_json(path: String) -> Dictionary:
    ...
    if path == VERSION_PATH:
        var hit: Variant = web.get("version", {})
