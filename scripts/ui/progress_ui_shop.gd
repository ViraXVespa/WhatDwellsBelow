...
static func rebuild_anvil(ui) -> void:
	ui._clear()
	ui.box.add_child(ThemeS.lab("Anvil", 32, Color(0.95, 0.82, 0.5)))
	var smith := App.prog.skill_lv("smith")
	ui.box.add_child(ThemeS.lab("Analyze → first forge (gold + ore + root) → cheaper re-forges. Smithing %d shortens the wait." % smith, 18, Color(0.82, 0.76, 0.66)))
	ui.box.add_child(ThemeS.lab("Bank %dg  %d ore  %d root   Carried %dg %d ore %d root" % [App.bank_gold, App.bank_ore, App.bank_root, App.gold, App.ore, App.prog.root], 18, Color(0.8, 0.85, 0.7)))
	ui.status = ThemeS.lab("", 20, Color(0.95, 0.8, 0.45))
	ui.box.add_child(ui.status)
	...
	for s in App.prog.SLOTS:
		...
		if h.size() > 0:
			...
			ui.box.add_child(ThemeS.lab("Holds %s: %s" % [s, ", ".join(names)], 18, Color(0.75, 0.85, 0.7)))
	ui.box.add_child(ThemeS.btn("Close  (B)", func(): ui.close_ui()))


static func anvil_pick(ui) -> void:
	ui.status.text = "Analyze a piece first."
...
static func anvil_analyze(ui) -> void:
	...
	var smith := App.prog.skill_lv("smith")
	ui.box.add_child(ThemeS.lab("Analysis — %s" % str(it.name), 26, Color(0.95, 0.86, 0.55)))
	ui.box.add_child(ThemeS.lab("Slot %s   Rarity %s   +%d dmg   +%d def   +%d HP" % [slot, str(it.get("rarity", "white")), int(it.get("dmg", 0)), int(it.get("def", 0)), int(it.get("hp", 0))], 18, Color(0.88, 0.82, 0.7)))
	var wait := App.prog.forge_duration()
	ui.box.add_child(ThemeS.lab("Smithing %d: cheaper, faster, better. Holds %d/3. Forge time %.1fs." % [smith, h.size(), wait], 18, Color(0.8, 0.85, 0.7)))
	if first:
		ui.status.text = "First forge: %dg  %d ore  %d root. A again to confirm." % [cost.gold, cost.ore, cost.root]
		ui.focus_btn = ThemeS.btn("First Forge  (confirm)", func(): ui._confirm(func(): do_forge(ui, it), "forge"))
	else:
		ui.status.text = "Re-forge: %dg  %d ore  %d root. A again to confirm." % [cost.gold, cost.ore, cost.root]
		ui.focus_btn = ThemeS.btn("Re-forge  (confirm)", func(): ui._confirm(func(): do_forge(ui, it), "forge"))
	ui.box.add_child(ui.focus_btn)
	ui.box.add_child(ThemeS.btn("Analyze another", func(): ui.anvil_item = {}; ui.pending = false; ui._rebuild_anvil(); ui._show()))
...
static func do_forge(ui, it: Dictionary) -> void:
	...
	ui._st("Forging… %.1fs. Smithing shortens this. B cancels." % ui.forge_t)
