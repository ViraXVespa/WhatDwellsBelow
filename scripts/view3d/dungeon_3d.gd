...
const Enemy3D := preload("res://scripts/view3d/enemy_3d.gd")
const Interact3D := preload("res://scripts/view3d/interact_3d.gd")
const Solid3D := preload("res://scripts/view3d/solid_3d.gd")
const Area3DS := preload("res://scripts/view3d/area_3d.gd")
		...
		for x in w:
			...
			if t == DungeonGen.FLOOR:
				...
				if near_floor:
					walls.append(pos)
	if not fa.is_empty():
		add_child(V3.tile_mm("res://assets/tiles/dungeon_floor.png", fa, 0.0))
	if not fb.is_empty():
		add_child(V3.tile_mm("res://assets/tiles/dungeon_floor_b.png", fb, 0.0))
	if not walls.is_empty():
		add_child(V3.wall_mm("res://assets/tiles/dungeon_wall.png", walls))
...
func _spawn_entities() -> void:
	_iact("floor_crystal", data.crystal, "res://assets/sprites/props/crystal.png")
	var st_locked := Game.run.current_floor >= DungeonGen.SLICE_MAX_FLOOR
	_iact("stairs", data.stairs, "res://assets/sprites/props/stairs.png", {"locked": st_locked})
	...
	for mp in data.mines:
		_iact("mining", mp, "res://assets/sprites/props/ore.png")
	...
	if data.has("loot"):
		for lp in data.loot:
			_iact("loot_stash", lp, "res://assets/sprites/props/chest.png")
	if data.has("breakables"):
	...
	if data.has("campfires"):
		for cp in data.campfires:
			_iact("campfire", cp, "res://assets/sprites/props/campfire.png")
	if data.has("shrines"):
	...
	if data.has("shop"):
		...
		if sp.x >= 0:
			_iact("ghost_shop", sp, "res://assets/sprites/npcs/shopkeep.png")
	if data.has("chests"):
		for cp in data.chests:
			_iact("artifact_chest", cp, "res://assets/sprites/props/chest.png")
	if data.has("fires"):
