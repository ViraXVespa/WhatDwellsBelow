...
const ProjS := preload("res://archives/art_experiment/scripts/view3d/projectile_3d.gd")
...
func _load_sprites() -> void:
	...
	for pose in ["idle", "windup", "strike"]:
		for dir in Art.FACING_KEYS:
			var path := V3.a("enemies", "%s/%s_%s.png" % [folder, pose, dir])
			if ResourceLoader.exists(path):
				sprites["%s_%s" % [pose, dir]] = load(path)
	for dir in ["down", "up", "left", "right"]:
		var frames: Array = []
		for i in 8:
			var wp := V3.a("enemies", "%s/walk_%s_%d.png" % [folder, dir, i])
