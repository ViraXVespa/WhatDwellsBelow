...
- Submenus keep the parent control strip and draw their own strip. No Back button. Binding UI: the gear ui door. Binding item / roll rules: the inventory door → the inventory / gear job.
...
- Occupancy, no knockback, refill at 0 HP: the combat door.
...
- Hidden-until-`update_line` / no cream-box flash: the combat door.
...
### Live Snapshot
- `App.play_from_menu()` / `enter_dungeon()` use loading overlay for hub/dungeon assets (the ui door).
