...
- Offline tools that read this file are listed in art_pipeline. A Regenerate flag on a player clip resolves to a specific `i2v_seeds.py` MOTION key (`attack_great_axe`, not a generic `attack`). A `gather` flag writes both pickaxe and hatchet prompts. Pack accepted one-shot I2V with `tools/pack_oneshot.py`.
